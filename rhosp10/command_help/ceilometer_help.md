...
Optional arguments:
  ...
  --os-user-id <user-id>        Defaults to env[OS_USER_ID].
  --os-user-domain-id <user-domain-id>
...
Optional arguments:
  ...
  --project-id <ALARM_PROJECT_ID>
                                ...
                                by admin users only).
  --user-id <ALARM_USER_ID>     User to associate with alarm (configurable by
                                admin users only).
...
```
usage: ceilometer alarm-create --name <NAME> [--project-id <ALARM_PROJECT_ID>]
                               [--user-id <ALARM_USER_ID>]
...
Optional arguments:
  ...
  -q <QUERY>, --query <QUERY>   key[op]data_type::value; list for filtering
...
Optional arguments:
  ...
  -o <ORDERBY>, --orderby <ORDERBY>
                                ...
                                direction}] The direction is one of: ['asc',
                                'desc'].
  -l <LIMIT>, --limit <LIMIT>   Maximum number of alarms to return. API server
                                limits result to <default_api_return_limit>
                                rows if no limit provided. Option is
                                configured in ceilometer.conf [api] group
```
...
Optional arguments:
  ...
  -o <ORDERBY>, --orderby <ORDERBY>
                                ...
                                direction}] The direction is one of: ['asc',
                                'desc'].
  -l <LIMIT>, --limit <LIMIT>   Maximum number of samples to return. API
...
Optional arguments:
  --project-id <SAMPLE_PROJECT_ID>
                                ...
                                by admin users only).
  --user-id <SAMPLE_USER_ID>    User to associate with sample (configurable by
                                admin users only).
  ...
  --direct <DIRECT>             Post sample to storage directly. Defaults to
                                False.
```
